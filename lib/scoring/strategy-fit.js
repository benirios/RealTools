...
const STRATEGY_FIT_CONFIG = {
  retail: {
    label: 'Varejo',
    categoryWeights: { location: 0.34, demographics: 0.24, commercial_activity: 0.30, risk_adjusted: 0.12 },
    factorWeights: { pedestrianTraffic: 0.20, visibility: 0.18, incomeCompatibility: 0.16, anchorProximity: 0.16, accessibility: 0.15, competitionBalance: 0.08, riskProfile: 0.07 },
  ...
  warehouse_logistics: {
    ...
    factorWeights: { roadAccess: 0.24, priceEfficiency: 0.20, industrialProximity: 0.18, parkingLoading: 0.18, accessibility: 0.10, riskProfile: 0.10 },
  ...
  rental_income: {
    ...
    factorWeights: { stableCommerce: 0.20, demandPredictability: 0.19, riskProfile: 0.18, anchorProximity: 0.15, pedestrianTraffic: 0.10, incomeCompatibility: 0.10, priceEfficiency: 0.08 },
  ...
  food_beverage: {
    ...
    factorWeights: { pedestrianTraffic: 0.20, officeResidentialDensity: 0.16, complementaryBusinesses: 0.16, visibility: 0.15, competitionBalance: 0.14, accessibility: 0.10, riskProfile: 0.09 },
  ...
  pharmacy: {
    ...
    factorWeights: { residentialDensity: 0.20, olderPopulation: 0.10, lowCompetition: 0.18, accessibility: 0.16, anchorProximity: 0.16, incomeCompatibility: 0.10, riskProfile: 0.10 },
  ...
  gym_fitness: {
    ...
    factorWeights: { residentialDensity: 0.18, incomeCompatibility: 0.16, parkingLoading: 0.14, accessibility: 0.12, complementaryBusinesses: 0.13, lowDirectCompetition: 0.15, riskProfile: 0.12 },
...
function getInsight(point) {
  return point.location_insight ?? point.locationInsight ?? point.insight ?? {}
}

function getListing(point) {
  return point.listing ?? point
}

function getNearby(insight) {
  const nearby = insight.nearbyBusinesses ?? insight.nearby_businesses
  return Array.isArray(nearby) ? nearby : []
}

function getInsightNumber(insight, camel, snake) {
...
function countNearby(nearby, terms, maxDistance = 800) {
...
function tagsText(listing) {
  const tags = Array.isArray(listing.tags) ? listing.tags.join(' ') : ''
  return normalize([tags, listing.title, listing.description, listing.property_type, listing.commercial_type].filter(Boolean).join(' '))
}

function scoreFromCount(count, one = 62, two = 78, many = 90) {
...
function scorePriceEfficiency(price) {
...
function confidenceFor(missingData, insightConfidence) {
...
function directCompetitionScore(count) {
...
function competitionBalanceScore(count) {
...
function addReason(target, condition, label) {
  if (condition && !target.includes(label)) target.push(label)
}

function calculateStrategyFitScore(point, strategy) {
  const strategySlug = STRATEGY_FIT_SLUGS.includes(strategy) ? strategy : 'retail'
