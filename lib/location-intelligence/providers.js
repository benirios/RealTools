...
async function getDemographicEstimate(location = {}, options = {}) {
  // Only include MockDemographicsProvider if explicitly requested via options
  const hasMockExplicit = Array.isArray(options.demographicProviders) && options.demographicProviders.some(p => p?.name === 'mock')
