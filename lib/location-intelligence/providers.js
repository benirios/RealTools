...
async function fetchMunicipalitiesForState(state, options = {}) {
  ...
  // 'BR' is the country code fallback used when no real state could be
  // determined — never a valid UF, so skip the doomed IBGE lookup entirely.
  if (!uf || uf.length !== 2 || uf === 'BR') return []
...
async function geocodeLocation(input = {}, options = {}) {
  ...
  // Mock/demo geocoding is dev-only — never fabricate a result for real user
  // requests. When no real provider resolves the address, report failure
  // (null) so callers can surface an "address not found" error, matching how
  // getDemographicEstimate already behaves for its own mock fallback.
...
async function getDemographicEstimate(location = {}, options = {}) {
  const providers = Array.isArray(options.demographicProviders) && options.demographicProviders.length > 0
    ...
    : [options.demographicProvider, SidraDemographicsProvider, ...(options.allowMock ? [MockDemographicsProvider] : [])].filter(Boolean)
    ...
    try {
      ...
      if (result) {
        return {
          ...
          provider: result.provider ?? provider.name ?? 'unknown',
          ...
          dataSources: [
            {
              ...
              fallback: (result.provider ?? provider.name ?? '') === 'mock',
              confidence: clampConfidence(result.confidence ?? 60),
            },
          ],
        }
      ...
      warnings.push(`${provider.name ?? 'demographics'}: ${error instanceof Error ? error.message : 'unknown error'}`)
    }
  }

  // No real data available — return error state instead of fabricating
  return null
}
...
async function resolveLocationIntelligence(input = {}, options = {}) {
  // Wire the Google Maps/Places key from the environment when a caller doesn't
  // pass one explicitly. Without this, options.googleMapsApiKey is always
  // undefined (no caller supplies it), so the Google geocode + nearby-business
  // paths never fire and every nearby-business card renders empty — the mock
  // fallback is stripped downstream by isReliableNearbyBusiness. Mirrors the
  // existing process.env.NOMINATIM_USER_AGENT fallback in getNominatimGeocodeResult.
  ...
  if (!geocode) {
    return {
      ...
      rawDemographics: {},
      rawPlaces: {},
      warnings: ['Endereço não encontrado. Verifique o endereço, bairro ou cidade informados.'],
      providers: { geocode: 'not_found', demographics: null, places: null },
      notFound: true,
    }
  }
  ...
  const consumerProfile = deriveConsumerProfile({
    ...
    populationDensity: demographics?.populationDensity ?? null,
  ...
  const confidenceScore = clampConfidence(
    Math.round((geocode.confidence + (demographics?.confidence ?? 0) + places.confidence) / 3)
  )
  ...
  return {
    ...
    providers: {
      ...
      demographics: demographics?.provider ?? null,
