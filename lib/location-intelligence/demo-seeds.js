...
async function createDemoLocationInsightsForListing(supabase, userId, listing) {
  ...
  const standalone = await resolveLocationIntelligence(standaloneInput, { allowMock: true })
  ...
  const linked = await resolveLocationIntelligence(linkedInput, { allowMock: true })
