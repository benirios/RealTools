...
import { AddImovelDialog } from '@/components/listings/add-imovel-dialog'
...
type ListingSummary = Pick<
  ListingRow,
  'id' | 'title' | 'price_text' | 'price_amount' | 'city' | 'neighborhood' | 'state' | 'address_text' | 'location_text' | 'commercial_type' | 'property_type' | 'confidence' | 'source' | 'source_url' | 'first_seen_at' | 'images'
> & {
...
const BASE_LISTING_COLUMNS = 'id, title, price_text, price_amount, city, neighborhood, state, address_text, location_text, commercial_type, property_type, confidence, source, source_url, first_seen_at, images'
...
function applyListingFilters(query: any, filters: ListingFilters) {
  ...
  } else if (filters.source === 'scraped') {
    nextQuery = nextQuery.neq('source', 'manual')
  }
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <form action="/imoveis" className="rounded-md border border-border bg-card p-4">
        <div className="grid gap-4 lg:grid-cols-[1.2fr_1.2fr_0.8fr_0.7fr_0.7fr_0.7fr_0.7fr_auto]">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <option value="scraped">Importado</option>
            </select>
          </div>
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-border bg-card px-6 py-16 text-center">
          ...
          <p className="mt-2 max-w-sm text-sm text-muted-foreground">
            {filtersActive ? (
              ...
              <>Use a aba <Link href="/listings/import" className="underline">Pesquisas</Link> para importar do OLX ou clique em <strong>Adicionar imóvel</strong> para inserir manualmente.</>
            )}
          </p>
        </div>
      ) : (
