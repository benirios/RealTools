...
export function registerDealTools(server) {
    server.tool("rt_list_deals", "List all CRE deals for the current user. Returns id, title, address, price, status, property_type, created_at.", {
        status: z
            .string()
            .optional()
    ...
    server.tool("rt_get_deal", "Get full deal details including notes and recent activity log.", { deal_id: z.string().uuid().describe("Deal UUID") }, async ({ deal_id }) => {
        ...
        if (dealRes.error)
            throw new Error(dealRes.error.message);
        return {
            content: [{
                    type: "text",
                    text: JSON.stringify({
    ...
    server.tool("rt_create_deal", "Create a new deal in RealTools.", {
    ...
    server.tool("rt_update_deal_status", "Change a deal's status.", {
    ...
    server.tool("rt_add_note", "Add a note to a deal.", {
        deal_id: z.string().uuid(),
