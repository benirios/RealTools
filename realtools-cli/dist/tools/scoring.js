...
export function registerScoringTools(server) {
    server.tool("rt_list_scores", "List opportunity scores across all listings, sorted by score descending.", {
        ...
        let q = db
            ...
            .limit(limit ?? 20);
        if (strategy_slug)
            q = q.eq("strategy_slug", strategy_slug);
        const { data, error } = await q;
        if (error)
            throw new Error(error.message);
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
    server.tool("rt_get_score", "Get full opportunity score breakdown (signals, risks, category scores) for a listing + strategy combination.", {
    ...
    server.tool("rt_top_listings", "Get top N listings ranked by opportunity score for a given business strategy.", {
        strategy_slug: z.enum(STRATEGY_SLUGS),
        ...
        const scoresRes = await db
            ...
            .limit(limit ?? 10);
        if (scoresRes.error)
            throw new Error(scoresRes.error.message);
        if (!scoresRes.data?.length) {
            return { content: [{ type: "text", text: `No listings scored above ${min_score} for strategy '${strategy_slug}'.` }] };
        }
        const listingIds = scoresRes.data.map((s) => s.listing_id);
        ...
        const result = scoresRes.data.map((score) => ({
            ...score,
            listing: listingMap[score.listing_id] ?? null,
        }));
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    });
    server.tool("rt_compare_strategies", "Compare opportunity scores for the same listing across all available strategies.", { listing_id: z.string().uuid() }, async ({ listing_id }) => {
        const { data, error } = await db
            .from("opportunity_scores")
        ...
        if (error)
            throw new Error(error.message);
        if (!data?.length) {
            return { content: [{ type: "text", text: "No scores computed for this listing. Run scoring from the RealTools UI first." }] };
        }
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    });
}
