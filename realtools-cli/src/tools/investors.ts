...
export function registerInvestorTools(server: McpServer) {
  server.tool(
    "rt_list_investors",
    "List all investors/buyers in the CRM with optional filters.",
    {
    ...
    async ({ strategy, risk_level, min_budget }) => {
      ...
      if (min_budget !== undefined) q = q.gte("budget_max", min_budget);
  ...
  server.tool(
    "rt_match_investors_for_listing",
    "Find investors whose criteria match a given listing. Uses listing price, property type, location, and best strategy scores to rank fit.",
    ...
    async ({ listing_id }) => {
      ...
      const topStrategies: string[] = (scoresRes.data ?? []).map((s: any) => s.strategy_slug);
      const investors: any[] = investorsRes.data ?? [];
      ...
      const matched = investors
        .map((inv) => {
          ...
          const neighborhoodMatch =
            !inv.preferred_neighborhoods?.length ||
            inv.preferred_neighborhoods.some((n: string) =>
