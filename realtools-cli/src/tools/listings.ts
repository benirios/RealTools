...
export function registerListingTools(server: McpServer) {
  server.tool(
    "rt_search_listings",
    "Search commercial property listings with filters. Returns title, address, price, classification, and source.",
    {
    ...
    async ({ city, state, is_commercial, min_price, max_price, property_type, limit }) => {
      ...
      if (is_commercial !== undefined) q = q.eq("is_commercial", is_commercial);
      if (min_price !== undefined) q = q.gte("price_amount", min_price);
      if (max_price !== undefined) q = q.lte("price_amount", max_price);
  ...
  server.tool(
    "rt_get_listing",
    "Get full listing details plus its location insight if enrichment has run.",
