...
export function registerLocationTools(server: McpServer) {
  server.tool(
    "rt_get_location_insight",
    "Get enriched demographic and commercial context for a listing's location. Includes avg income, population density, nearby businesses, and consumer profile.",
    { listing_id: z.string().uuid() },
    async ({ listing_id }) => {
      const { data, error } = await db
        .from("location_insights")
        .select(
          "id,address,city,state,neighborhood,latitude,longitude," +
  ...
  server.tool(
    "rt_listings_missing_insight",
    "Find listings that have no location insight yet — candidates for enrichment.",
    {
    ...
    async ({ city, state, limit }) => {
      ...
      const enrichedIds: string[] = (insightsRes.data ?? []).map((r: any) => r.listing_id);
      ...
      if (enrichedIds.length) q = q.not("id", "in", `(${enrichedIds.join(",")})`);
